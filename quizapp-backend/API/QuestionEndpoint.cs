...
namespace quizapp_backend.API
{
    public static class QuestionEndpoint
    {
        ...
        {
            IEnumerable<Question> questions = await questionRepository.Get();

            IEnumerable<OutputQuestion> outputQuestion = QuestionDtoManager.Convert(questions);
            Payload<IEnumerable<OutputQuestion>> payload = new Payload<IEnumerable<OutputQuestion>>(outputQuestion);
        ...
        public static async Task<IResult> Create(IRepository<Question> questionRepository, InputQuestion inputQuestion)
        {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        public static async Task<IResult> Update(IRepository<Question> questionRepository, int id, InputQuestion inputQuestion)
        {
            Question? question = await questionRepository.Get(id);
            if (question is null)
                return TypedResults.NotFound();
